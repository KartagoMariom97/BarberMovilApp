...
interface AdminClientApi {
    ...
    @GET("admin/clients")
    suspend fun getAllClients(): ApiResponse<List<AdminClientResponse>>

    /** Crea un nuevo cliente con cuenta de usuario vía admin */
    @POST("admin/clients")
    suspend fun createClient(@Body request: AdminCreateClientRequest): ApiResponse<ClientResponse>

    @GET("admin/clients/{id}")
    suspend fun getClientById(@Path("id") id: Long): ApiResponse<AdminClientResponse>

    @PUT("admin/clients/{id}")
    suspend fun updateClient(
        @Path("id") id: Long,
        @Body request: AdminUpdateClientRequest,
    ): ApiResponse<AdminClientResponse>
